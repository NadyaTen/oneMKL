...
bool test(const device& dev, onemkl::uplo upper_lower, onemkl::transpose transa,
    ...
    try {
#ifdef CALL_RT_API
        onemkl::blas::tbsv(main_queue, upper_lower, transa, unit_nonunit, n, k, A_buffer, lda,
                           x_buffer, incx);
#else
        TEST_RUN_CT(
            main_queue, onemkl::blas::tbsv,
            (main_queue, upper_lower, transa, unit_nonunit, n, k, A_buffer, lda, x_buffer, incx));
#endif
    }
    catch (exception const& e) {
